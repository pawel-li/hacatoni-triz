...
@Component({
  ...
  template: `
    <main
      class="flex min-h-dvh flex-col bg-[#0f1110] px-4 pt-4 text-[#efe8da] sm:px-8 lg:grid lg:min-h-dvh lg:grid-cols-2 lg:items-center lg:px-12"
      aria-label="Home"
    >
            ...
            <div class="flex flex-col gap-2">
              <div class="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <span class="text-[0.6rem] leading-tight opacity-60">Inspiracja natura</span>
                </button>
                <button
                  type="button"
                  id="method-triz"
                ...
                >
                  ...
                  <span class="text-[0.6rem] leading-tight opacity-60">Matryca sprzecznosci</span>
                </button>
              </div>
            </div>
...
export class HomeComponent {
  ...
  toggleMethod(method: 'biomimicry' | 'triz'): void {
    if (method === 'biomimicry') {
      this.useBiomimicry.update((value) => !value);
    } else {
      this.useTriz.update((value) => !value);
    }
  }

  getToggleClass(active: boolean): string {
    const base =
      "flex cursor-pointer flex-col items-center gap-1 rounded-none border border-dotted px-3 py-2.5 font-['Inter'] transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-40";
    if (active) {
