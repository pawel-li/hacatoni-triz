...
from ai_agent.triz import stream_triz_run
...
def _load_dotenv():
    ...
    env_path = next((parent / ".env" for parent in resolved.parents if (parent / ".env").is_file()), None)
    if env_path is None:
        return
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if method == "triz":
                for event in stream_triz_run(prompt):
