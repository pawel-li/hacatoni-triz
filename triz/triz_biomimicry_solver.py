...
def generate_biomimicry_solutions(problem: str) -> str:
    prompt = f"""
    You are an R&D engineer specializing in Biomimicry. 
    Generate at least 3 distinct candidate solutions for the following problem.
    
    Problem: {problem}
    
    You MUST base your solutions on "The Nine Principles of Biomimicry":
    ...
    Do not use paper bags. The solutions must disappear or be reused responsibly.
    For each candidate, explicitly state which of the Nine Principles it uses in the `principle_used` field.
    """
    return call_llm(prompt, response_schema=Solutions)

def evaluate_candidates(problem: str, triz_sols: str, biomimicry_sols: str) -> str:
    prompt = f"""
    ...
    Biomimicry Solutions: {biomimicry_sols}
...
def select_best_candidate(problem: str, evaluations: str) -> str:
    ...
    Explain your full reasoning trail: why this one was chosen over the others, and how it perfectly solves the problem.
    """
...
def main():
    ...
    print("=== TRIZ & Biomimicry R&D Innovation System ===\n")
    ...
    print("Step 3: Generating Biomimicry Candidate Solutions...")
    biomimicry_solutions = generate_biomimicry_solutions(problem_statement)
    print(json.dumps(json.loads(biomimicry_solutions), indent=2))
    print("\n" + "-"*50 + "\n")
    
    print("Step 4: Evaluating All Candidates...")
    evaluations = evaluate_candidates(problem_statement, triz_sols=triz_solutions, biomimicry_sols=biomimicry_solutions)
